...
| 你的 Emby | 用哪种 | 为什么 |
|---|---|---|
| 局域网、`http://` | **A · 局域网自托管** | 两边都是 http、同一内网,没有混合内容墙 |
| 公网、有域名和证书 `https://` | **B · Pages 托管** | 两边都是 https,浏览器放行 |
| 想一劳永逸、还想免掉跨域 | **C · 同源反代** | 同一个域名,混合内容和 CORS 两道墙一起消失 |

### A · 局域网自托管(本地 Emby 用这个)
...
把 LinWeb 和 Emby 挂到同一个域名的不同路径下,浏览器视为同源,**连 CORS 都不用配**。
...
除了方式 C,其余方式 LinWeb 和 Emby 都不同源,Emby 必须返回跨域头。在 Emby 后台
...
完整教程、Caddy/nginx 示例、常驻运行(PM2 / systemd / Docker)见
**[DEPLOY.md](DEPLOY.md)**。
...
<summary><b>目录结构</b></summary>
    ...
    tools/test-touch.mjs    手机手势:CDP 派发合成触摸(唯一可无头的浏览器测试)
...
`test-touch.mjs` 是唯一允许跑无头的浏览器测试:它一帧都不解码,所以无头 Chromium
缺少 HEVC 解码器这件事与"一次长按有没有把倍速推到 2×"无关。它自己拉起 serve.mjs
和无头 Edge,用 CDP 的 `Input.dispatchTouchEvent` 打真实触摸。截图能证明控制条是
46px,只有派发出去的触摸能证明手势接住了它 —— 两者缺一不可。
