...
镜像是 **nginx alpine-slim + 构建产物**,约 53 MB,跑起来单个 worker、常驻内存
10 MB 上下。镜像里没有 Node、没有 npm,构建阶段用完就丢。
...
53 MB 里有 **31 MB 是单个文件**:`vendor/ffmpeg-core.wasm`,浏览器一律不解的
E-AC3 / AC-3 / DTS / TrueHD 的软解兜底。片库里没有这些音轨就整块扔掉:

```bash
docker build --build-arg WITH_FFMPEG=0 -t linweb .     # 约 22 MB
```
